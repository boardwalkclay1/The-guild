...
const PRICE_TICK_MS = 220;

// Level 3: true arena, big swings
const scenarios = [
  {
    id: 0,
    type: "example",
...
function showExampleOverlay() {
  ...
  overlayContent.innerHTML = `
    <h2>Level 3 – The Arena</h2>
    <p>
      This is closer to the real battlefield. Spreads whip, prices spike, and hesitation is punished.
    </p>
    <p>
      Example:<br>
    ...
    <p>
      The rules have not changed. Only the violence has.
      You still start with <strong>$100</strong>.
    </p>
...
function showFailureOverlay(message) {
  overlayContent.innerHTML = `
    <h2>Trial Failed</h2>
    <p>${message}</p>
    <p>You must restart Level 3 from the beginning.</p>
...
function showSuccessOverlay() {
  overlayContent.innerHTML = `
    ...
    <p>
      You have survived the Arena. Only the Death Strike remains.
    </p>
...
function goToNextLevel() {
  window.location.href = "arena-level4.html";
}
...
function startPriceEngine(scenario) {
  ...
  priceInterval = setInterval(() => {
    ...
    bid += (Math.random() - 0.5) * v * 2;
    ask += (Math.random() - 0.5) * v * 2;
...
openTicketBtn.addEventListener("click", () => {
  ...
  if (scenario.type === "guided" && phase === "buy") {
    const suggestedContracts = 3;
    const suggestedPrice = mid.toFixed(2);
    orderMessageEl.innerHTML =
      `For this first trial of Level 3, enter <strong>${suggestedContracts}</strong> contracts at the MID <strong>$${suggestedPrice}</strong>.`;
  }
});
